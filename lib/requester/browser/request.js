...
/* eslint-disable */ // @todo - move this file to vendor or fix linting issues
...
function request(options, callback) {
    ...
    if(!options) {
        return callback(new Error('No options given'));
    }
    ...
    for (var i = 0; i < unsupported_options.length; i++)
        if(options[ unsupported_options[i] ])
            console.warn("options." + unsupported_options[i] + " is not supported")
    ...
    var multipart = function (obj) {
        ...
        _.forOwn(obj, function (value, key) {
            _.isArray(value) ? _.each(value, function (eachValue) {
            ...
            for (var key in options.headers) {
                if (options.headers.hasOwnProperty(key)) {
                    if (Array.isArray(options.headers[key])) {
                        _.each(options.headers[key], function (eachValue) {
