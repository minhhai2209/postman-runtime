(function(universe) {
    ...
    __delete && __delete.before && __delete.before.length && __delete.before.forEach(function(key) {
        delete universe[key];
    });
    ...
    for (g in universe.__globals) {
        if (universe.__globals.hasOwnProperty(g)) {
            scope.replace(g, universe.__globals[g]);
        }
    }
    ...
    for (lib in libraries) {
        if (libraries.hasOwnProperty(lib)) {
            scope.replace(lib, libraries[lib]);
        }
    }
    ...
    for (g in replacements) {
        if (replacements.hasOwnProperty(g)) {
            scope.replace(g, replacements[g]);
        }
    }
    ...
    __delete.final && __delete.final && __delete.final.length && __delete.final.forEach(function(key) {
        delete universe[key];
    });
}(this));
